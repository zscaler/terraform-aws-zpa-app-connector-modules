...
resource "aws_subnet" "ac_subnet" {
  ...
  availability_zone = data.aws_availability_zones.available.names[count.index]
  cidr_block        = var.ac_subnets != null ? element(var.ac_subnets, count.index) : cidrsubnet(data.aws_vpc.vpc_selected.cidr_block, 8, count.index + 200)
  vpc_id            = data.aws_vpc.vpc_selected.id
...
data "aws_subnet" "ac_subnet_selected" {
  ...
  id    = var.byo_subnets == false ? aws_subnet.ac_subnet.*.id[count.index] : element(var.byo_subnet_ids, count.index)
}
...
resource "aws_route_table_association" "ac_rt_asssociation" {
  count          = length(data.aws_subnet.ac_subnet_selected.*.id)
  subnet_id      = data.aws_subnet.ac_subnet_selected.*.id[count.index]
  route_table_id = aws_route_table.ac_rt.*.id[count.index]
}
