...
# Or use existing Provisioning Key if specified in byo_provisioning_key
data "zpa_provisioning_key" "provisioning-key-selected" {
  name             = var.byo_provisioning_key == false ? zpa_provisioning_key.provisioning-key.name : var.byo_provisioning_key_name
  association_type = var.provisioning_key_association_type
}